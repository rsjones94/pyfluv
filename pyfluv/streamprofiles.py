...
class Profile():
    ...
    def __init__(self, df, name=None, metric=False):
        ...
        if not 'Station' in self.df:
            ...
            #actualy a gross way to keep this code from running when the subclass Feature is instantiated
            self.validate_df()
            self.update_filldf()
                ...
                if not pd.isnull(self.filldf[col][i]):
                    ...
                    if all(neighbor is False for neighbor in neighbors):
                        ...
                        if modifydf and modifyfilldf:
                            ...
                            logging.warning('Isolated %s call on row %d in Profile %s. Call will be deleted from df and filldf.', col, i, self.name)
                        elif modifydf:
                            self.df[col][i] = np.NaN
                            logging.warning('Isolated %s call on row %d in Profile %s. Call will be deleted from df.', col, i, self.name)
                        elif modifyfilldf:
                            self.filldf[col][i] = np.NaN
                            logging.warning('Isolated %s call on row %d in Profile %s. Call will be deleted from filldf.', col, i, self.name)
    ...
    def trend(self, col, order=1, updateLegend=True):
        ...
        """
        x = np.linspace(self.filldf['Station'].iloc[0], self.filldf['Station'].iloc[-1], self.length())
    ...
    def make_poly_string(self, poly):
        """
    ...
    def ordered_features(self):
        """
        Flattens self.features and returns it as a list sorted by start station.
        """
        orderedFeats = []
    ...
    def spacing(self, featureType, spacingFrom='deepest', spacingTo='deepest',
        ...
        for i, _ in enumerate(self.features[featureType]):
            if i == length-1:
                break
            sta1 = self.features[featureType][i].feature_measurepoint(measureType=spacingFrom, deepType=deepType)
            sta2 = self.features[featureType][i+1].feature_measurepoint(measureType=spacingTo, deepType=deepType)
    ...
    def resort_features(self):
        """
        Makes sure self.features is has features under the correct key and
        are ordered by their beginning station.
        """
    ...
    def xsind(self, CrossSection):
        ...
        """
        crossseg = CrossSection.crossseg()
        for index, _ in self.filldf.iterrows():
            try:
                segment = self._segment(index)
                if sm.does_intersect(segment, crossseg):
                    return index
            except IndexError:
                pass
    ...
    def classify_by_adjacency(self, priority='next'):
        ...
        for i, feat in enumerate(feats):
            if feat.morphType == 'Unclassified':
                try:
                    if i+priStep < 0:
                        raise IndexError
    ...
    def feature_classify_k_means(self, nClass=2, thalwegSmooth=(1, 0), waterSmooth=(1, 0),
        ...
        for i, _ in self.filldf.iterrows():
            morph = morphs[i]
...
class Feature(Profile):
    ...
    def feature_measurepoint(self, measureType, deepType=None):
        """
